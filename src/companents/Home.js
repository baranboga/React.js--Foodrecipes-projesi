...
<div>
  ...
  <div className="row justify-content-center">
    <div className="col-lg-6 mt-5">
      <form className="d-flex" role="search">
        <input type="search" className="form-control form-control-dark text-bg-dark me-2" placeholder="search for recipes" aria-label="Search" style={{ color: "black", backgroundColor: "yellow" }} onChange={(e) => {
          setkey(e.target.value)
        }} />
        <button className='btn btn-warning' style={{ color: "black", backgroundColor: "yellow", fontFamily: 'fantasy' }} onClick={(e) => {
          e.preventDefault()
          ara(key)
        }}>Search</button>
      </form>
  ...
  <div className="row justify-content-center ms-5" >
    <div className="col-12 d-flex flex-wrap" style={{justifyContent:"space-between"}}>
      ...
      <button id='b' type="button" className="btn btn-warning me-2 mb-2" style={{ borderRadius: "10px" }} onClick={(e) => {
        const buttonElement = document.getElementById("b");
        const buttonText = buttonElement.innerText;
        tıkla(buttonText)
      }}>Dessert</button>
      <button id='c' type="button" className="btn btn-success me-2 mb-2" style={{ borderRadius: "10px" }} onClick={(e) => {
        const buttonElement = document.getElementById("c");
        const buttonText = buttonElement.innerText;
        tıkla(buttonText)
      }}>Vegan</button>
      <button id='d' type="button" className="btn btn-danger me-2 mb-2" style={{ borderRadius: "10px" }} onClick={(e) => {
        const buttonElement = document.getElementById("d");
        const buttonText = buttonElement.innerText;
        tıkla(buttonText)
      }}>Beef</button>
      <button id='e' type="button" className="btn btn-primary me-2 mb-2" style={{ borderRadius: "10px" }} onClick={(e) => {
        const buttonElement = document.getElementById("e");
        const buttonText = buttonElement.innerText;
        tıkla(buttonText)
      }}>Breakfast</button>
      <button id='f' type="button" className="btn btn-info me-2 mb-2" style={{ borderRadius: "10px" }} onClick={(e) => {
